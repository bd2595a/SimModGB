...
int palette[4];
int color, tileset, tilemap, scrollx, scrolly;
int tileposition, tileaddress, tileindex, tilex, tiley;
int row0, row1, row0shifted, row0capturepixel, row1shifted, row1capturepixel;
int pixel, x, y, xoffset, yoffset;
...
void renderScreen()
{
	...
	for (int i = 0; i<144; i++)
	{
		for (int j = 0; j<160; j++)
		{
			...
			std::cout << "y: " << y << " x: " << x << " scrolly: " << scrolly << " scrollx: " << scrollx << std::endl;
...
{
	setup(argc, argv);
	std::ifstream romfile("TETRIS.GB", ios::in | ios::binary | ios::ate);
